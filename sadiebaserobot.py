...
class BaseRobot:
    ...
    def waitForMillis(self, millis):
        ...
        """
        Makes the robot not do anyting unntill the time is up
    ...
    def waitForForwardButton(
        ...
        """
        Makes the robot not do anyting unntill the forward button is pressed
    ...
    def waitForBackButton(
        ...
        while True:
            ...
            wait(10)
        """
        Makes the robot not do anyting unntill the back button is pressed
    ...
    def turnInPlace(
        ...
        """
        turns the robot while staying in the same spot

        Example

        >>> br.turnInPlace(angle=90)
        >>> br.turnInPlace(angle=-70, speedPct=60)
        >>> br.turnInPlace(angle=50, accelerationPct=70)

        angle (REQUIRED, integer) How much of a turn positive = right; \
        negative = left

        speedPct (pos integer, optional): How fast to turn. Defaults to \
        DEFAULT_BIG_MOT_SPEED_PCT.
